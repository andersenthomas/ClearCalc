...
if($connection_error){
	...
	$root_dir = '/srv/www/htdocs/clearance';
	$directory = tempnam(sys_get_temp_dir(), $date . '-' . $timestamp . '-'); 
	$current_date_dir = '/srv/www/htdocs/clearance/patient_ris_data' . $directory;

    get_RIS_data(True, get_current_date('Ymd'));
    
    $dcm_files = return_files_directory($root_dir, 'dcm');
    create_directory($current_date_dir);
    move_files($dcm_files, $root_dir, $current_date_dir);
    convert_dcm2txt($dcm_files, $current_date_dir);
    $txt_files = return_files_directory($current_date_dir, 'txt');  
    
    //iterate over all the files in the directory
    $number_of_inserts = 0;
    $number_of_moves = 0;
    for ($i=0; $i<sizeof($txt_files); $i++){
        $file = file_get_contents($current_date_dir . '/' . $txt_files[$i]);
        ...
        if(substr($data[11],0,8) == 'Glomerul'){//GFR patient
			...
			$patient_age = calc_age($data[8], $date, '%a')/365.25;
			...
			if(strpos($data[11], 'flere') != false) {//24h exam
				if($patient_age > 15.0){
					...
					SELECT "' . $data[5] . '", "' . $data[6] . '", "' . $data[7] . '", "' . $date . '", "' . $creation_date . '", "' . $data[9] . '", "V3", "' . $data[8] . '", "' . $data[10] . '" FROM patient_data 
					WHERE NOT EXISTS(SELECT accession_number FROM patient_data WHERE accession_number = "' . $data[5] . '") LIMIT 1';
				} else {
					$query_patient_data = 'INSERT INTO patient_data (accession_number, patient_name, patient_cpr, patient_exam_date, creation_date, patient_sex, measurement_type, patient_birthday, department) 
					SELECT "' . $data[5] . '", "' . $data[6] . '", "' . $data[7] . '", "' . $date . '", "' . $creation_date . '", "' . $data[9] . '", "B3", "' . $data[8] . '", "' . $data[10] . '" FROM patient_data 
					WHERE NOT EXISTS(SELECT accession_number FROM patient_data WHERE accession_number = "' . $data[5] . '") LIMIT 1';
				}
			} else {//1point exam
				if($patient_age > 15.0){
					$query_patient_data = 'INSERT INTO patient_data (accession_number, patient_name, patient_cpr, patient_exam_date, creation_date, patient_sex, measurement_type, patient_birthday, department) 
					SELECT "' . $data[5] . '", "' . $data[6] . '", "' . $data[7] . '", "' . $date . '", "' . $creation_date . '", "' . $data[9] . '", "V1", "' . $data[8] . '", "' . $data[10] . '" FROM patient_data 
					WHERE NOT EXISTS(SELECT accession_number FROM patient_data WHERE accession_number = "' . $data[5] . '") LIMIT 1';
				} else {
					$query_patient_data = 'INSERT INTO patient_data (accession_number, patient_name, patient_cpr, patient_exam_date, creation_date, patient_sex, measurement_type, patient_birthday, department) 
					SELECT "' . $data[5] . '", "' . $data[6] . '", "' . $data[7] . '", "' . $date . '", "' . $creation_date . '", "' . $data[9] . '", "B1", "' . $data[8] . '", "' . $data[10] . '" FROM patient_data 
					WHERE NOT EXISTS(SELECT accession_number FROM patient_data WHERE accession_number = "' . $data[5] . '") LIMIT 1';
				}
			}
    ...
    echo "----------\n";
    echo "Success at " . $date . " " . $timestamp . "\n";
    echo $number_of_inserts . " patients inserted\n";
    echo $number_of_moves . " patients moved\n";
    echo "----------\n";
    system('/bin/rm -rf ' . escapeshellarg($current_date_dir));
}
